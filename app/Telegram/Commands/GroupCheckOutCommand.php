...
class GroupCheckOutCommand extends BaseCommand
{
    public function authorize(TelegramUser $tgUser): bool
    {
        return parent::authorize($tgUser) && ($tgUser->user?->can('groups.checkout') ?? false);
    }

    public function handle(array $args, TelegramUser $tgUser, ?TelegramConversationState $state): void
    {
        if (! $this->requirePermission($tgUser, 'groups.checkout')) {
            return;
        }

        if (empty($args)) {
            $this->reply($tgUser, 'Usage: /groupcheckout <group_code>');
        ...
        try {
            $results = app(GroupCheckOutAction::class)($group, $tgUser->user);
        ...
        $message = "✅ Group check-out complete for {$code}.\n{$success} room(s) checked out.";

        if ($failed > 0) {
            $failures = collect($results['failed'])
                ->map(fn ($f) => "• {$f['reservation_code']} room {$f['room_number']}: {$f['reason']}")
                ->implode("\n");
            $message .= "\n\n⚠️ {$failed} failed:\n{$failures}";
        }
