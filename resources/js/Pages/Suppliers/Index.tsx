...
export default function SuppliersIndex({ suppliers }: SuppliersIndexProps) {
    ...
    return (
        <AuthenticatedLayout title="Suppliers">
            ...
            <ProTable rowKey="id" search={false} options={false} pagination={{ showTotal: (total, range) => `${range[0]}-${range[1]} of ${total}` }} dataSource={suppliers.data} columns={columns} scroll={{ x: 'max-content' }} />
