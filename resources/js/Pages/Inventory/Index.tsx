...
export default function InventoryIndex({ items, filters, categoryOptions, unitOptions, lowStockCount }: InventoryIndexProps) {
    ...
    return (
        <AuthenticatedLayout title="Inventory">
            ...
            <ProTable rowKey="id" search={false} options={false} pagination={{ showTotal: (total, range) => `${range[0]}-${range[1]} of ${total}` }} dataSource={items.data} columns={columns} scroll={{ x: 'max-content' }} />
