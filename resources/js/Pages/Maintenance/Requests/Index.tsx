...
export default function MaintenanceRequestsIndex({ requests, filters, statusOptions, priorityOptions }: MaintenanceRequestsIndexProps) {
    ...
    return (
        <AuthenticatedLayout title="Maintenance Requests">
            ...
            <ProTable rowKey="id" search={false} options={false} pagination={{ showTotal: (total, range) => `${range[0]}-${range[1]} of ${total}` }} dataSource={requests.data} columns={columns} scroll={{ x: 'max-content' }} />
        </AuthenticatedLayout>
    );
}
