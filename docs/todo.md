...
- Reservation/billing document flow is at phase 3: the Proforma Invoice (phase 1), down payments + finance verification + Payment Receipts + outstanding tracking (phase 2), and the release-gated guest Invoice built from folio items (phase 3). Booking expiry auto-cancel and carrying a verified proforma payment into the folio are later phases and deliberately absent. Run `php artisan db:seed --class=RolePermissionSeeder` on existing databases to create the `proforma.release`, `proforma.payment.record`, `proforma.payment.verify` and `invoice.release` permissions.
