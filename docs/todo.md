...
- `[done] P0: Room and F&B revenue is PBJT 10% tax inclusive, not service charge + PPN 11% on top - was overcharging by ~21% [app/Support/TaxAmountCalculator.php (extractInclusive/inclusiveFactor), app/Services/TaxCalculator.php, app/Services/FolioPostingService.php, app/Models/FolioItem.php (line_total, dpp_amount), add_is_tax_inclusive_to_folio_items_table + replace_ppn_tax_rule_with_pbjt + add_pbjt_payable_account_to_chart_of_accounts migrations, app/Listeners/PostFolioChargeToGl.php, app/Services/Reports/PbjtReport.php, app/Console/Commands/ResplitInclusiveTaxCommand.php (billing:resplit-inclusive-tax), guest documents stripped of SC/Tax columns, tests/Feature/InclusiveTaxTest.php + ResplitInclusiveTaxCommandTest.php] (completed: 2026-09-22)`
...
- Room and F&B prices are tax inclusive: the price list figure stays in `folio_items.amount`/`unit_price`, `service_charge_amount` and `tax_amount` hold the carved-out SC 10% and PBJT 10%, and `is_tax_inclusive` makes the line total the amount alone. Never sum `amount + tax_amount + service_charge_amount` without the flag — use `FolioItem::line_total` or `FolioPostingService::LINE_TOTAL_SUM_SQL`. Guest documents print the flat price with no SC or tax column; the split is reporting-only and shows up in the PBJT recap on `/accounting/tax`. Existing databases need `php artisan migrate` (replaces the `ppn` rule with `pbjt` and adds CoA `2-2110 PBJT Terutang`) and then `php artisan billing:resplit-inclusive-tax --dry-run` before a real run; the command deliberately does not touch the general ledger, so the overstated revenue and tax on historical folios still need a reversing journal entry from Finance.
